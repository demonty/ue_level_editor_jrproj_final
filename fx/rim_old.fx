...
/* ~~~~~~~~~~ Static Variables ~~~~~~~~~~ */
static float gRimIntensity = 1.0f;
static float gRimStart = 0.0f;
static float gRimEnd = 0.25f;
...
{	
	...
	float rim = smoothstep(gRimStart, gRimEnd, 1 - dot(n,v));
	
	return float4(c , 1.0f) + float4(rim * gColor * gRimIntensity, 1.0f);
}
